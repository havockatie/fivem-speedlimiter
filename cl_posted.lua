    ...
    while true do
        ...
        if IsPedInAnyVehicle(ped) then
            ...
            if (activate == true) then 
                speedlimiter = true
                SetVehicleMaxSpeed(GetVehiclePedIsIn(ped, false), meterspeed)
        end
            if (IsDisabledControlJustPressed(0, 74)) then
                if speedlimiter == true then
                    SetVehicleMaxSpeed(GetVehiclePedIsIn(ped, false), 89.4)  
                    speedlimiter = false
                    activate = false
                    Notify('~g~Speed Limiter has been activated')
        elseif speedlimit == "~r~~h~Contact Developmentt!" then
            SetVehicleMaxSpeed(GetVehiclePedIsIn(ped, false), 89.4)
        elseif speedlimiter == false then
            activate = true
        Notify('~r~Speed Limiter has been deactivated')
                end
            end
        end
    end
